...
def _build_remap_fn(gguf_keys: List[str], arch: str = "unknown"):
    ...
    # Check if Krea2 format
    elif any(k.startswith("txtfusion.") for k in gguf_keys) or any(k.startswith("blocks.0.attn.qknorm.") for k in gguf_keys):
        max_i = 0
        for k in gguf_keys:
            parts = k.split(".")
        ...
        def _remap(name: str):
            return remap.get(name, [(name, None)])
        logger.info("[GGUFSeeker] Krea2 / Minimax H3 key naming detected — remapping to Diffusers convention.")
        return _remap
