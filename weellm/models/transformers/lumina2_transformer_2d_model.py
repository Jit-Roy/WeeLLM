...
Lumina2 uses a lazy-init pattern (nn.Module subclass) because it requires
``trust_remote_code=True`` and has context_refiner / noise_refiner / layers as
three separately streamed block groups.
"""
...
class Lumina2Transformer2DModelStreamer(nn.Module):
    ...
    Uses lazy initialization because the model requires ``trust_remote_code=True``.
    ...
    def _ensure_initialized(self) -> None:
        ...
        logger.info("Initialising streaming Lumina2 transformer ...")
        ...
        for buf_name, buf in list(self.model.named_buffers()):
            if buf.device.type != self.device:
                set_module_tensor_to_device(
                    self.model, buf_name, self.device,
                    value=buf.to(self.dtype) if buf.is_floating_point() else buf,
                )
        ...
        if self.prefetch:
            self._executor = ThreadPoolExecutor(
                max_workers=1, thread_name_prefix="lumina_gpu_load"
            )

        self._install_hooks()
        self._initialized = True
        logger.info("Installed %d transformer blocks for streaming.", len(self.layer_sequence))
    ...
    def forward(self, *args, **kwargs):
        ...
        with self._lock:
            ...
            if self._executor is not None and self.layer_sequence:
                first_idx  = self.layer_sequence[0]
                first_keys = _get_layer_keys(self._seeker, first_idx)
                self._next_future     = self._executor.submit(
                    self._seeker.get_tensors, first_keys, self.device, self.dtype
                )
                self._next_future_idx = first_idx
