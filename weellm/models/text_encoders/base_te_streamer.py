...
class BaseLazyDecoderStreamer(ABC):
    ...
    Subclasses MUST implement:
      ...
      - ``_layer_prefix(idx)``  → weight-map key prefix for layer *idx* (e.g. ``"model.layers.3."``)
      - ``_resident_key_filter(key)`` → True if *key* belongs to the resident (always-on-GPU) set
      - ``_get_model_layers()`` → the ``nn.ModuleList`` of transformer layers on ``self._model``
      - ``_load_model_skeleton()`` → instantiate ``self._model`` on meta device, set ``self._num_layers``
      - ``_load_resident_extra()`` → optional post-step after resident weights are placed
        (e.g. rotary embedding fix-ups); default is a no-op
      - ``_capture_layer_indices()`` → set of layer indices whose outputs should be captured
    ...
    def _capture_layer_indices(self) -> set:
        """Return the set of layer indices whose hidden-state outputs should be captured."""
    ...
    def _layer_pre_hook(self, module: nn.Module, args):
        ...
        if next_idx < self._num_layers and self._executor is not None:
            next_prefix = self._layer_prefix(next_idx)
            next_keys   = [k for k in self._seeker.weight_map if k.startswith(next_prefix)]
            with self._lock:
