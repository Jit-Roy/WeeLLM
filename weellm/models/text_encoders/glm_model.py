...
class GlmModelStreamer:
    """
    Hook-based streaming text encoder (GlmModel).
    ...
    def __init__(
        ...
        extract_layers: Tuple[int, ...] = (39,),  # For GlmModel (40 layers), we capture output of layer 39
        device: str = "cuda",
    ...
    def _ensure_initialized(self):
        if self._initialized:
            return
        print("Initialising streaming GLM text encoder ...")
        ...
        self._executor = ThreadPoolExecutor(
            max_workers=1, thread_name_prefix="glm_gpu_load"
        )
        self._initialized = True
        print("GLM Text encoder ready (streaming via Live Seek).")
        ...
        for buf_name, buf in list(rotary.named_buffers()):
            if buf.device.type != self.device:
                set_module_tensor_to_device(
                    self._model, f"rotary_emb.{buf_name}",
    ...
    def encode(self, prompt: str) -> torch.Tensor:
        ...
        # stacked shape: (B, num_captured, seq, hidden)
