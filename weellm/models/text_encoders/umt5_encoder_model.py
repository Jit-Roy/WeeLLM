"""
umt5_streamer.py -- Hook-based layer-streaming for UMT5EncoderModel.
...
UMT5 encoder block structure in safetensors is identical to T5:
...
class UMT5EncoderModelStreamer:
    ...
    def _pre_hook(self, module: nn.Module, args):
        ...
        module._umt5_loaded_params = list(sd.keys())

    def _post_hook(self, module: nn.Module, args, output):
        _evict_params(self.model, getattr(module, "_umt5_loaded_params", []))
        module._umt5_loaded_params = []
        return output
    ...
    def from_pretrained(
        ...
        print(f"Initializing SafetensorsLiveSeeker on UMT5 text_encoder weights ...")
        ...
        print(f"Loading resident UMT5 tensors to GPU ({len(resident_keys)} tensors) ...")
