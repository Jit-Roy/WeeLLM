...
class HiDreamImageEditingPipeline(DiffusionPipeline, HiDreamImageLoraLoaderMixin):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # --- NATIVE IMAGE RESIZING LOGIC ---
        # Automatically resizes images to preserve aspect ratio while ensuring the 
        # total sequence length (patches) does not exceed the model's hard limit.
        # Note: In editing mode, HiDream concatenates the source image and latent side-by-side (dim=-1), 
        # doubling the sequence length. Thus, the single image can only have at most 4608 / 2 = 2304 patches.
        import PIL.Image
        import math

        def _resize_to_fit(img):
            if isinstance(img, PIL.Image.Image):
                # 2304 max patches * (8x VAE * 2x packing)^2 = 589,824 max pixels
                max_pixels = 2304 * 256 
                w, h = img.size
                if w * h > max_pixels:
                ...
                # Must be strictly divisible by 16 for VAE decoding and 2x2 patch packing
                new_w = max(16, (new_w // 16) * 16)
                new_h = max(16, (new_h // 16) * 16)
                
                if new_w != w or new_h != h:
                    logger.info(f"Auto-resizing image from {w}x{h} to {new_w}x{new_h} to maintain aspect ratio within 2304 max patches per image.")
                    img = img.resize((new_w, new_h), PIL.Image.LANCZOS)
            return img

        if image is not None:
            if isinstance(image, list):
                image = [_resize_to_fit(img) for img in image]
            else:
        ...
        # --- AUTO-LOAD HIDREAM-E1-FULL REFINEMENT LORA ---
        if refine_strength == 0.0 and getattr(self, "transformer", None) is not None:
            import os
            from pathlib import Path
            if hasattr(self, "_name_or_path") and self._name_or_path:
                lora_path = Path(self._name_or_path) / "HiDream-E1-Full.safetensors"
                # Avoid loading if already loaded (we can check if reload_keys is not None, or if we have a flag)
                if lora_path.exists() and reload_keys is None:
                    if not getattr(self, "_e1_lora_loaded", False):
                        from safetensors.torch import load_file
                        try:
                            lora_ckpt = load_file(str(lora_path), device="cpu")
                            
                            # The streamer needs the LoRA weights for dynamic subtraction
                            if hasattr(self.transformer, "load_lora"):
                                self.transformer.load_lora(str(lora_path))
                            
                            non_lora_keys = [k for k in lora_ckpt if "lora" not in k]
                            if non_lora_keys:
                                src_state_dict = {}
                                # Try to fetch resident keys from the model
                                tr_model = getattr(self.transformer, "model", None)
                                if tr_model is not None:
                                    src_state_dict = tr_model.state_dict()
                                
                                self._cached_reload_keys = {
                                    "editing": {k: lora_ckpt[k] for k in non_lora_keys},
                                    "refine": {k: src_state_dict.get(k, lora_ckpt[k]) for k in non_lora_keys},
                                }
                                logger.info("  Auto-Enabled HiDream-E1-Full Refine Mode (Refine strength: 0.3)")
                            self._e1_lora_loaded = True
                        except Exception as e:
                            logger.warning(f"Failed to auto-load E1 LoRA for refinement: {e}")
                    
                    if getattr(self, "_cached_reload_keys", None) is not None:
                        reload_keys = self._cached_reload_keys
                        refine_strength = 0.3
        # -------------------------------------------------
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if reload_keys is not None and i == int(num_inference_steps * (1.0 - refine_strength)):
                    ...
                    if hasattr(self.transformer, "set_refine_mode"):
                        self.transformer.set_refine_mode(True)
                ...
                with torch.autocast("cuda", dtype=transformer_func.dtype):
